...
ref = db.reference().child(accountname)
ref.update({'Gender':b})
times = str(int(time.time()))
ref = db.reference().child(accountname).child(times)
...
while tracking == True:
    ...
    if (x == 1 and mb_angle < -70 and mb_angle > -140 ):
        x = 2
    if (x == 2 and mb_angle < 120 and mb_angle > 0 ):
        x = 1
        rep = rep + 1
        print(rep)
        ref = db.reference().child(accountname).child(times)
...
ref.update({'duration':totaltime})


#print(type(totaltime))
#print(type(Cals))
#print(type(accountname))
#print(type(mb_weight_recived))
#print(type(times))
#print(type(rep))